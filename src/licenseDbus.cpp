...
void removeTempFiles()
{
    for (const auto& entry : fs::directory_iterator(UPLOADED_NEWKEYPATH))
    {
        if (fs::is_regular_file(entry.path()))
        {
            ...
            {
                std::cerr << "Error removing file " << entry.path() << ": "
                          << e.what() << std::endl;
            }
        }
    }
                      ...
                      const std::string& NewtokenString)
{
    ...
    std::string newTimeStamp =
        extractDataFromToken(NewtokenString, "TIMESTAMP");
    ...
    if ((convertTimeFormat(currentTimeStamp) >
         convertTimeFormat(newTimeStamp)) &&
        ((convertTimeFormat(currentTimeStamp) >
          convertTimeFormat(oldEffectiveTimestamp)) &&
...
int ValidateMacAddr(const std::string& LicMacAddr)
{
    ...
    {
        std::cerr << "D-Bus method call error: " << reply.get_error()
                  << std::endl;
        return -1;
    }
...
int updateNewServiceValidity(const std::string& inputString, json& jsonData)
{
    ...
    {
        int existingServiceLicenseValidity =
            existingServiceInfo["LicenseValidity"];
        ...
        DEBUG(std::cout << "OldexistingLicenseValidity "
                        ...
                        << " BMC have run existing upCountDays "
                        << extupCountDays << " remining license validity "
                        << (existingServiceLicenseValidity +
                            extGlobalLicenseValidity) -
                               extupCountDays
        ...
        for (auto& serviceInfo : jsonData["licensableServices"])
        {
            if (serviceInfo["serviceName"] ==
                existingServiceInfo["serviceName"])
            {
...
std::string generateServiceValidityString(const json& jsonData)
{
    ...
    for (auto& service : licensableServices)
    {
        ...
        if (licenseValidity > 0)
        {
            ...
            ValidityString +=
                serviceName + COLON + std::to_string(licenseValidity);
        }
    }
                                       ...
                                       const std::string& str2)
{
    ...
    while (std::getline(iss2, segment_str2, SEMICOLON))
    {
        ...
        if (pos != std::string::npos)
        {
            ...
            str1.replace(startPos, endPos - startPos,
                         std::to_string(serviceValue));
        }
        else
        {
            ...
            str1 = serviceName + COLON + std::to_string(serviceValue) +
                   SEMICOLON + str1;
        }
    }
...
int updateLicTokenwithExtValidity()
{
    ...
    {
        std::cerr << "Error: VALIDITY section not found in the file."
                  << std::endl;
        return -1;
    }
    ...
    if (!outputFile)
    {
        std::cerr << "Error: Cannot open output file "
                  << licenseValidityTokenPath << " for writing." << std::endl;
        return -1;
    }
...
bool enableServices(const std::string& serviceName)
{
    std::vector<std::string> systemCtlServiceNames =
        getSystemCtlServiceNames(serviceName);
                ...
                service["serviceName"].is_string())
            {
                    ...
                    service["LicenseValidity"].is_number())
                {
                    ...
                    if (serviceLicenseValidity != 0)
                    {
                        ...
                        {
                            std::cerr << "Error enabling service '"
                ...
                {
                    std::cerr << "Invalid JSON format for service '"
                              << serviceName << "'. Skipping." << std::endl;
                }
            }
        }
    }
...
std::string generateTotalValidityData()
{
    ...
    {
        validityData += service.first + COLON + std::to_string(service.second) +
                        SEMICOLON;
    }
...
int64_t LicenseControlImp::servicesUpCountDays()
{
    return UpCountDays;
}
...
bool LicenseControlImp::addLicenseKey()
{
    return writeLicenseKeyToFile();
}
