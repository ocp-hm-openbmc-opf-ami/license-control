...
std::vector<std::string> appendServiceTypeToCommands(
        ...
        for (const auto& service : globalData["licensableServices"])
        {
            if (service["serviceName"] == ServiceNames)
            {
                ...
                if (UpCountDays >= (licenseValidity + Globalvalidcount))
                {
                    ...
                    for (const auto& SystemCtlServiceName :
                         SystemCtlServiceNames)
                    {
                        DEBUG(std::cout << SystemCtlServiceName << " ";);
                        controlSystemdService(SystemCtlServiceName,
                                              ServiceAction::Stop);
                    }
                ...
                else
                {
                    ...
                    {
                        if (((Globalvalidcount + licenseValidity) -
                             UpCountDays) >= 0)
                        {
                            serviceDates[ServiceNames] = std::to_string(
                                (Globalvalidcount + licenseValidity) -
                                UpCountDays);
                        }
                        else
                        {
                    ...
                    DEBUG(std::cout << "Service " << ServiceNames
                                    << " is within the license validity."
                                    << std::endl;);
                }
            }
        }

        DEBUG(std::cout << "Number of services in the JSON: "
                        << numServicesInJson << std::endl;);
...
int checkValidity()
{
    ...
    {
        std::cout << "AMI Global Licence period expired.. Exiting!"
                  << std::endl;
        return 1;
    }
    return 0;
...
int updateValidityFromLicensePeriod(const std::string& licensePeriod)
{
    ...
    while (startPos < licensePeriod.size())
    {
        ...
        {
            std::string service =
                licensePeriod.substr(startPos, colonPos - startPos);
            int validity = 0;
            if (semicolonPos != std::string::npos)
            {
                validity = std::stoi(licensePeriod.substr(
                    colonPos + 1, semicolonPos - colonPos - 1));
            }
            else
            {
            ...
            DEBUG(std::cout << "service -" << service << " validity "
                            << validity << std::endl;);
    ...
    if (std::filesystem::exists(licenseValidityTokenPath))
    {
        ...
        if (fileStream.is_open())
        {
            ...
            do
            {
                ...
                attempt++;
                std::cout << "Invalid BMC Time Stamp. Attempt " << attempt
                          << " of " << maxRetries << std::endl;
