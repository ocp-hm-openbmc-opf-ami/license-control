...
int getPublicKey()
{
    ...
    DEBUG(std::cout << "pub_key_identifier: " << pub_key_identifier
                    << "\nLC_PK_IDENTIFIER: " << LC_PK_IDENTIFIER
                    << std::endl;);
    if (strncmp(pub_key_identifier, LC_PK_IDENTIFIER,
                sizeof(LC_PK_IDENTIFIER)) != 0)
    {
    ...
    {
        std::cerr << "Error: Unable to open file " << licensePublicKey
                  << std::endl;
        if (MTDDevId >= 0)
        {
            close(MTDDevId);
                             ...
                             const std::string& tag)
{
    ...
    {
        std::cerr
            << "Integrity Error: Invalid data length, unauthorized changes"
            << std::endl;
        return "";
    }
...
std::string generatedHashValue(const std::string& input)
{
    ...
    if (!mdCtx)
    {
        std::cerr << "Error: Unable to create message digest context"
                  << std::endl;
        return "";
    }
...
int decriptLicenseFile(const std::string& encryptLicencefile)
{
    ...
    if (!newLicenseToken.is_open())
    {
        std::cerr << "Error: Unable to open file "
                  << newLicenseValidityTokenPath << std::endl;
